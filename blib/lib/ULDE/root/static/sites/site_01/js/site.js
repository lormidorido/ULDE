// JavaScript Document$(function(){
	...
	$( document ).ready(function() {
		...
		$(".popup-content.selector .question").tooltip();
/*		
        $( "#accordion" ).accordion({
			active: 0,
			collapsible: false,
...
//		$(".box-content div.sub-header").button();
			
		$(".popup.youtube").colorbox({iframe:true, innerWidth:640, innerHeight:390, opacity:0.95});
	
		$(".selector.popup").colorbox({inline:true, width:750, scrolling:false, opacity:0.95,
								onComplete:function(){ if ($("#colorbox").find(".no-notifications").length > 0) { $.colorbox.resize({ width:524 }) } } 
		});
			
		$("#products .popup").colorbox({inline:true, width:670, opacity:0.95});
		
/*		$("#banner-txt .popup").colorbox({inline:true, width:670, opacity:0.95}); */
		
		$("#footer-nav .popup").colorbox({iframe:true, innerWidth:"667px", height:"50%", opacity:0.95});
	
		$(".radio" ).buttonset();
/*		
		$(".selector #info_accordion").accordion({
			heightStyle: "content"
			});
*/
		$(".popup-content.selector .selector.next").click(function() {
			if ($(this).parents(".popup-content.selector").find("input:radio:checked").length != $(this).parents(".popup-content.selector").find(".radio").length) {
				fields_required($(this).parents(".control-bar"));
				return false;
			}
			
			var href = $(this).parents(".popup-content.selector").find("input:radio:checked").filter(function() {
	 ...
	 $(".popup-content.selector input:radio").click(function() {
		update_notification($(this));
		if ($(this).parents(".popup-content.selector").find("input:radio:checked").length == $(this).parents(".popup-content.selector").find(".radio").length) {
			fields_selected($(this).parents(".popup-content.selector").find(".control-bar"));
		}
	 });
	 
	 // Display initial notifications
	 $(".popup-content.selector").children(".notification.show").css('visibility', 'visible')


    });
		
	$(window).load(function() {
    	$('#slider_01, #slider_02').nivoSlider({
			pauseTime: 6000,	
			effect: 'random'
		});
	});
	...
	function fields_required( obj ) {
		obj.removeClass("ui-state-highlight");
		obj.addClass("ui-state-error");
		obj.find("p span").removeClass("ui-icon-info");
		obj.find("p span").addClass("ui-icon-alert");	 
	}

	 function fields_selected( obj ) {
		obj.addClass("ui-state-green");
		obj.removeClass("ui-state-error");
//		$("#root p span").addClass("ui-icon-check");
//		$("#root p span").removeClass("ui-icon-alert");
		obj.find("p").html('<span class="ui-icon ui-icon-check"></span>Please click "Next" to proceed!');
	 }

	 function update_notification( obj ) {
		if (!obj.val().match(/^#/)) {
			if (obj.val()) {
				if (!notifications[obj.parents(".popup-content.selector").attr("id")]) {
					notifications[obj.parents(".popup-content.selector").attr("id")] = [];
				} else if (notifications[obj.parents(".popup-content.selector").attr("id")][notifications[obj.parents(".popup-content.selector").attr("id")].length-1] == obj.attr("name")) {
					return;
				} else {
					var index = notifications[obj.parents(".popup-content.selector").attr("id")].indexOf(obj.attr("name"));
					if (index > -1) {
						notifications[obj.parents(".popup-content.selector").attr("id")].splice(index,1);
					}					
				}				
				obj.parents(".popup-content.selector").find(".notification-text").html('<span class="ui-icon ui-icon-info"></span>'+obj.val());
				obj.parents(".popup-content.selector").children(".notification").css('visibility', 'visible');
				notifications[obj.parents(".popup-content.selector").attr("id")].push(obj.attr("name"));
			} else {
				if (notifications[obj.parents(".popup-content.selector").attr("id")] && notifications[obj.parents(".popup-content.selector").attr("id")].length > 0) {
					if (notifications[obj.parents(".popup-content.selector").attr("id")][notifications[obj.parents(".popup-content.selector").attr("id")].length-1] == obj.attr("name")) {
						notifications[obj.parents(".popup-content.selector").attr("id")].pop();
						obj.parents(".popup-content.selector").children(".notification").css('visibility', 'hidden');
						if (notifications[obj.parents(".popup-content.selector").attr("id")].length > 0) {
							update_notification($("input:radio[name="+notifications[obj.parents(".popup-content.selector").attr("id")].pop()+"]:checked"));
						}
					} else { 
						var index = notifications[obj.parents(".popup-content.selector").attr("id")].indexOf(obj.attr("name"));
						if (index > -1) {
